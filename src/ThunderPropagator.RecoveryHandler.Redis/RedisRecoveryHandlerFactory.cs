...
internal sealed class RedisRecoveryHandlerFactory(
...
) : IRecoveryHandlerFactory
{
    public bool CanHandle(IChannel channel) =>
        channel.Metadata.Snapshot.RecoveryStorage == RecoveryStorage.Redis;

    public IRecoveryHandler Create(IChannel channel) =>
        featureGate.IsAllowed<RedisRecoveryStorageFeature>()
            ? new RedisRecoveryHandler(serviceProvider, channel)
            : noneHandler;
}
