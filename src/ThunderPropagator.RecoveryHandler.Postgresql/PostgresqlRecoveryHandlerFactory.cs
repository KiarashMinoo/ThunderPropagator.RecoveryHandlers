...
internal sealed class PostgresqlRecoveryHandlerFactory(
...
) : IRecoveryHandlerFactory
{
    public bool CanHandle(IChannel channel) =>
        channel.Metadata.Snapshot.RecoveryStorage == RecoveryStorage.Postgresql;

    public IRecoveryHandler Create(IChannel channel) =>
        featureGate.IsAllowed<PostgresqlRecoveryStorageFeature>()
            ? new PostgresqlRecoveryHandler(serviceProvider, channel)
            : noneHandler;
}
